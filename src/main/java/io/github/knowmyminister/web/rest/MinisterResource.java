...
public class MinisterResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new ministerDTO, or with status 400 (Bad Request) if the minister has already an ID
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated ministerDTO,
     * or with status 400 (Bad Request) if the ministerDTO is not valid,
     * or with status 500 (Internal Server Error) if the ministerDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    ...
    public ResponseEntity<MinisterDTO> updateMinister(@RequestBody MinisterDTO ministerDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, ministerDTO.getId().toString()))
            .body(result);
    }
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of ministers in body
     */
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the ministerDTO, or with status 404 (Not Found)
     */
    ...
    /**
     * SEARCH  /_search/ministers?query=:query : search for the minister corresponding
     * to the query.
     *
     * @param query the query of the minister search
     * @param pageable the pagination information
