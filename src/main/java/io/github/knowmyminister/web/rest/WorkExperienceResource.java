...
public class WorkExperienceResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new workExperienceDTO, or with status 400 (Bad Request) if the workExperience has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/work-experiences")
    @Timed
    public ResponseEntity<WorkExperienceDTO> createWorkExperience(@Valid @RequestBody WorkExperienceDTO workExperienceDTO) throws URISyntaxException {
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated workExperienceDTO,
     * or with status 400 (Bad Request) if the workExperienceDTO is not valid,
     * or with status 500 (Internal Server Error) if the workExperienceDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/work-experiences")
    @Timed
    public ResponseEntity<WorkExperienceDTO> updateWorkExperience(@Valid @RequestBody WorkExperienceDTO workExperienceDTO) throws URISyntaxException {
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the workExperienceDTO, or with status 404 (Not Found)
     */
    ...
    /**
     * SEARCH  /_search/work-experiences?query=:query : search for the workExperience corresponding
     * to the query.
     *
     * @param query the query of the workExperience search
     * @param pageable the pagination information
    ...
    public ResponseEntity<List<WorkExperienceDTO>> searchWorkExperiences(@RequestParam String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/work-experiences");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
