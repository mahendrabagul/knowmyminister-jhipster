...
@Component({
    selector: 'jhi-education-knowmyminister',
    templateUrl: './education-knowmyminister.component.html'
})
export class EducationKnowmyministerComponent implements OnInit, OnDestroy {

currentAccount: any;
    educations: EducationKnowmyminister[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/education-knowmyminister'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/education-knowmyminister', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: EducationKnowmyminister) {
        return item.id;
    }
