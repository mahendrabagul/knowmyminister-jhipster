...
@Component({
    selector: 'jhi-country-knowmyminister',
    templateUrl: './country-knowmyminister.component.html'
})
export class CountryKnowmyministerComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/country-knowmyminister'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/country-knowmyminister', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: CountryKnowmyminister) {
        return item.id;
    }
