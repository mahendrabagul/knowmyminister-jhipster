...
@Component({
    selector: 'jhi-language-knowmyminister',
    templateUrl: './language-knowmyminister.component.html'
})
export class LanguageKnowmyministerComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/language-knowmyminister'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/language-knowmyminister', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: LanguageKnowmyminister) {
        return item.id;
    }
