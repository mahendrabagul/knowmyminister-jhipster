...
@Component({
    selector: 'jhi-state-knowmyminister',
    templateUrl: './state-knowmyminister.component.html'
})
export class StateKnowmyministerComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/state-knowmyminister'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/state-knowmyminister', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: StateKnowmyminister) {
        return item.id;
    }
