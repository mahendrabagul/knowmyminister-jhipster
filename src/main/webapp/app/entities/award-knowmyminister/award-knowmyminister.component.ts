...
@Component({
    selector: 'jhi-award-knowmyminister',
    templateUrl: './award-knowmyminister.component.html'
})
export class AwardKnowmyministerComponent implements OnInit, OnDestroy {

currentAccount: any;
    awards: AwardKnowmyminister[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/award-knowmyminister'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/award-knowmyminister', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: AwardKnowmyminister) {
        return item.id;
    }
